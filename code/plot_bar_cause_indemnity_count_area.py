...
#    ax3.set_title('Hectares lost due to damage (2001-2016)')
...
def make_plot_subplot():
    ...
    fig, axes = plt.subplots(3,3, figsize=(12,15.5))
   ...
   # Row 1: corn
    for i,c in enumerate(crop_names):
        ...
        (bin_cause_area/1e6*0.404686).plot.bar(ax=axes[i,2], width=0.5)

        axes[i,0].set_title('The total amount of loss')
        axes[i,0].set_ylabel('Indemnity amount (billion US dolars)')

        axes[i,1].set_title('The total count')
        axes[i,1].set_ylabel('Count (thousand)')

        axes[i,2].set_title('Hectares lost due to damage (2001-2016)')
        axes[i,2].set_ylabel('Determined hectares (million)')
        if c=='corn':
            axes[i,0].text(0.7,0.9,'Maize',transform=axes[i,0].transAxes,fontweight='bold')
            axes[i,1].text(0.7,0.9,'Maize',transform=axes[i,1].transAxes,fontweight='bold')
            axes[i,2].text(0.7,0.9,'Maize',transform=axes[i,2].transAxes,fontweight='bold')
        else:
    ...
    for i in range(9):   
        ...
        # Add panel label 
        axes.flatten()[i].text(-0.09, 1.05, l[i], fontsize=14, transform=axes.flatten()[i].transAxes,
                               fontweight='bold')    
    
    sns.despine()
    plt.subplots_adjust(left=0.075, right=0.975, wspace=0.2, hspace=0.975, top=0.975, bottom=0.16)
